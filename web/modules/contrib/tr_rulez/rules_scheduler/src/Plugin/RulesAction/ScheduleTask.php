...
class ScheduleTask extends RulesActionBase {
  ...
  public function validate(RulesPlugin $element) {
    ...
    if (!$component) {
      throw new IntegrityException($this->t('The component %config does not exist.', ['%config' => $element->settings['component']]), $element);
    }
    // Check if the component is marked as dirty.
    rules_config_update_dirty_flag($component);
    if (!empty($component->dirty)) {
      throw new IntegrityException($this->t('The utilized component %config fails the integrity check.', ['%config' => $element->settings['component']]), $element);
    }
  }
  ...
  public function rules_scheduler_action_schedule_form_alter(&$form, &$form_state, $options, RulesAbstractPlugin $element) {
    ...
    $form['reload'] = [
      ...
      '#type' => 'submit',
      '#name' => 'reload',
      '#value' => $first_step ? $this->t('Continue') : $this->t('Reload form'),
